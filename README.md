...
* ALT Y: 切换持续翻译模式(开/关)
* ALT ENTER: 发音
* ENTER: 输出翻译文本（持续模式下自动重开翻译框）
* CTRL ENTER: 输出原始文本（持续模式下自动重开翻译框）
* ESC: 退出/关闭持续翻译模式
* TAB: 切换另一个翻译API
* CTRL F7: 网页版调试
#### 网页版额外快捷键
...
## 🚀 本 Fork 增强功能 (与源头项目的区别)

本项目基于 [sxzxs/Real-time-translation-typing](https://github.com/sxzxs/Real-time-translation-typing) 进行二次开发，新增以下功能：

### 1. 持续翻译模式 (Continuous Input Mode)
> 新增快捷键 **`Alt + Y`** 切换持续翻译模式

| 模式 | 行为 |
|------|------|
| **关闭**（默认） | 每次翻译完成后翻译框关闭，需重新 `Alt+Y` 打开 |
| **开启** | 按 `Enter` 发送翻译后，翻译框**自动重新弹出**，可连续翻译 |

- **开启持续模式**: 按 `Alt + Y`，提示「持续翻译模式 已开启」
- **关闭持续模式**: 按 `ESC` 或再次按 `Alt + Y`
- 在持续模式下，每次翻译结果输出后无需重新打开翻译框，大幅提升连续翻译效率

### 快捷键变更

| 快捷键 | 源头项目 | 本 Fork |
|--------|----------|---------|
| `Alt + Y` | 打开翻译器 | **切换持续翻译模式(开/关)** |
| `ESC` | 退出翻译器 | **退出/关闭持续翻译模式** |
